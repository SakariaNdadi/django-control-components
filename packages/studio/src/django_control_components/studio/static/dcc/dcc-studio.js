	...
	function register() {
		...
		window.Alpine.data("dccStudioDoc", (bootId) => ({
			...
			_post(url, extra) {
				const body = new URLSearchParams(
					Object.assign(
						{ doc: JSON.stringify(this.doc), revision: this.revision },
						extra || {},
					),
				);
				return fetch(url, {
			...
			save() {
				...
				this._post(this._cfg.saveUrl)
					...
					.then(({ ok, status, data }) => {
						...
						if (ok) {
							this.revision =
								data.revision != null ? data.revision : this.revision;
							this.dirty = false;
							document.body.dispatchEvent(
								new CustomEvent("dcc:toast", { detail: "Saved" }),
							);
							this.refreshPreview();
						} else if (status === 409) {
							this.error =
								"Someone else saved a newer version - reload to continue.";
							if (data.doc) {
								this.doc = data.doc;
		...
		window.Alpine.data("dccTree", (bootId) => ({
			...
			init() {
				...
				this.root =
					root && root.type
						? root
						: { id: this._newId(), type: "AppShell", props: {}, slots: {} };
				this._syncRaw();
				this.snapshot(true);
			},
			...
			blockInfo(type) {
				return (
					((this.palette && this.palette.blocks) || []).find(
						(b) => b.name === type,
					) || null
				);
			},
			...
			_locate(id, node, parent, slot) {
				node = node || this.root;
				if (node.id === id)
					return { node: node, parent: parent || null, slot: slot || null };
			...
			selectedNode() {
				return this.selectedId
					? (this._locate(this.selectedId) || {}).node || null
					: null;
			},
			select(id) {
				this.selectedId = id;
			...
			addChild(type) {
				...
				const slot =
					slots.indexOf(this.activeSlot) >= 0 ? this.activeSlot : slots[0];
			...
			remove(id) {
				...
				hit.parent.slots[hit.slot] = hit.parent.slots[hit.slot].filter(
					(n) => n.id !== id,
				);
				if (this.selectedId === id) this.selectedId = null;
				this.snapshot();
			...
			moveNode(id, toParentId, toSlot, toIndex) {
				...
				const destHit =
					this.root.id === toParentId
						? { node: this.root }
						: this._locate(toParentId);
				if (!destHit) return;
				...
				src.parent.slots[src.slot] = src.parent.slots[src.slot].filter(
					(n) => n.id !== id,
				);
		...
		window.Alpine.data("dccTreeDnd", () => ({
			...
			_target(e) {
				...
				).find((n) => n.getAttribute && n.hasAttribute("data-dcc-slot"));
				if (!el) return null;
		...
		window.Alpine.data("dccSortable", () => ({
			...
			_start(e) {
				...
				const end = (ev) => {
					...
					this.dragIndex = null;
					if (to != null)
						this.$dispatch("dcc-reorder", { from: this._origin, to: to });
				};
